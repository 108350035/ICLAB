...
reg [inst_sig_width+inst_exp_width:0] add0_a,add0_b,add1_a,add1_b,add2_a,add2_b;
wire [inst_sig_width+inst_exp_width:0] mul0_out,mul1_out,mul2_out,mul3_out,mul4_out,mul5_out,mul3_a,mul4_a,mul5_a,mul345_b;
reg [inst_sig_width+inst_exp_width:0] y10,y11,y12,y20,sigma10,sigma11,sigma12,temp0,temp1,temp2,temp3;
wire [inst_sig_width+inst_exp_width:0] add0_out,add1_out,add2_out,ReLU,dReLU,sub0_out;
...
DW_fp_mult#(inst_sig_width,inst_exp_width,inst_ieee_compliance)  M6(.a(learning_rate),.b(32'h3F000000),.rnd(3'b0),.z(new_rate));
...
assign ReLU = (y10[31])?0:y10;
assign dReLU = (ReLU==0)?0:ONE;
...
begin
    ...
    else if(in_valid_d | cs == F_2)  begin
        temp0<=mul0_out;
        temp1<=mul1_out;
        temp2<=mul2_out;
    end
...
begin
    ...
    else if(cs == U_1 && cnt == 0 ) out_valid<=1;
    else out_valid<=0;
end
...
begin
    if(!rst_n) iter<=0;
    else if(iter == 400 | in_valid_w1) iter<=0;
    else if(out_valid) iter<=iter+1;
end

always@(posedge clk)
begin
    if(in_valid_w1) learning_rate<=learn;
